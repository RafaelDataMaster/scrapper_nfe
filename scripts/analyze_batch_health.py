...
class BatchHealth:
    ...
    problemas: List[Tuple[str, str, float]] = field(default_factory=list)  # (tipo, descricao, severidade)
    ...
    def eh_empresa_interna(self) -> bool:
        ...
        empresas_internas = [
            "CARRIER", "RBC", "CSC", "MOC", "EXATA", 
            "ATIVE", "ORION", "ITACOLOMI", "FLORESTA", "SUNRISE"
        ]
...
class BatchHealthAnalyzer:
        ...
        try:
            with open(self.csv_path, "r", encoding="utf-8") as f:
                ...
                for i, row in enumerate(reader, 1):
                    try:
                        valor_str = row.get("valor_compra", "0").replace(".", "").replace(",", ".")
                        ...
                        batch = BatchHealth(
                            ...
                            valor_compra=valor,
                            valor_boleto=float(row.get("valor_boleto", "0").replace(".", "").replace(",", ".") or 0),
    ...
    def load_metadata(self, batch: BatchHealth) -> None:
        """Carrega metadata.json para um batch."""
    ...
    def detectar_problemas(self, batch: BatchHealth) -> None:
        ...
        if batch.tem_valor and len(batch.fornecedor.strip()) < 10 and batch.valor_compra > 1000:
            # Exceções: empresas conhecidas com nome curto
            excecoes = ["GOX", "CEMIG", "COPASA", "EDP"]
        ...
        if batch.valor_compra > 10000:
            nomes_pessoa = ["marco", "túlio", "lima", "natalia", "ferreira", "silva", "raphaela"]
        ...
        # === PROBLEMA 7: Valor > 0 mas sem vencimento (quando parece ser boleto) ===
        if batch.tem_valor and not batch.vencimento.strip():
            # Verificar se assunto sugere boleto/fatura
            assunto_upper = batch.email_subject.upper()
            if any(termo in assunto_upper for termo in ["BOLETO", "FATURA", "VENCIMENTO", "PAGAMENTO"]):
                batch.problemas.append((
                    "VENCIMENTO_AUSENTE",
        ...
        # === PROBLEMA 9: Status CONFERIR mas valores OK (informacional) ===
        if batch.status_conciliacao == "CONFERIR" and batch.tem_valor and batch.tem_fornecedor_valido:
            batch.problemas.append((
                "STATUS_CONFERIR",
                "Status CONFERIR mas dados parecem OK (pode ser falta de boleto para conciliação)",
                self.SEV_BAIXA
            ))
    ...
    def generate_report(self, output_path: Optional[Path] = None) -> str:
        ...
        lines = []
        lines.append("=" * 100)
        lines.append("RELATÓRIO DE SAÚDE DOS BATCHES - ANÁLISE DE EXPORTAÇÃO")
        lines.append("=" * 100)
        lines.append("")
        ...
        com_problemas = sum(1 for b in self.batches if b.problemas)
        ...
        lines.append(f"Batches com valor exportado: {com_valor} ({com_valor/total*100:.1f}%)")
        lines.append(f"Batches com fornecedor válido: {com_forn_valido} ({com_forn_valido/total*100:.1f}%)")
        lines.append("")
        lines.append(f"Batches com problemas: {com_problemas} ({com_problemas/total*100:.1f}%)")
        ...
        lines.append("DISTRIBUIÇÃO DE PROBLEMAS")
        lines.append("-" * 100)
        ...
        for tipo, ocorrencias in sorted(problemas_agrupados.items(), 
                                         key=lambda x: len(x[1]), reverse=True):
            valor_total = sum(b.valor_compra for b, _, _ in ocorrencias)
            lines.append("")
            lines.append(f"{tipo}: {len(ocorrencias)} ocorrências | Valor total: R$ {valor_total:,.2f}")
            
            # Mostrar exemplos (máximo 3)
            for batch, desc, sev in ocorrencias[:3]:
                lines.append(f"  - {desc}")
                lines.append(f"    Batch: {batch.batch_id} | Valor: R$ {batch.valor_compra:,.2f}")
        ...
        lines.append("TOP 10 PROBLEMAS CRÍTICOS (por valor)")
        lines.append("-" * 100)
        ...
        for batch in self.batches:
            for tipo, desc, sev in batch.problemas:
                if sev >= self.SEV_ALTA:
                    todos_problemas.append((batch.valor_compra, batch, tipo, desc))
        
        todos_problemas.sort(key=lambda x: x[0], reverse=True)
        
        for valor, batch, tipo, desc in todos_problemas[:10]:
            lines.append("")
            lines.append(f"Valor: R$ {valor:,.2f}")
...
def main():
    """Função principal."""
    print("=" * 100)
    print("ANÁLISE DE SAÚDE DOS BATCHES")
